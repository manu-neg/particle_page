...
import useParticleCanvasContext from "./Particle/ParticleCanvasContext";
...
export default function ParticlesCanvasV2() {
    ...
    function shuffleParticles(): void {
        setParticles(stateMemory.map(
            p => {
                const x = Math.ceil(Math.random() * 800); // TODO Assuming canvas size
    ...
    useEffect(() => {
        // Generate new particles with the factory to ensure they have the correct settings
        console.log("Updating particle amount to:", settings.particleAmount);
        if (particlesMeshRef.current) {
            let newP: Particle[] = [];
            for (let i = 0; i < settings.particleAmount - particles.length; i++) {
                particlesMeshRef.current.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
                newP.push(factory.createParticle(
                    new Vector(
                        Math.random() * particlesMeshRef.current.scale.x, 
                        Math.random() * particlesMeshRef.current.scale.y
                    )
                ));
            }
            setParticles(newP);
    ...
    return (
        <div className="w-full h-full bg-transparent">
            ...
            <Canvas className="w-full h-full border-2 border-red-500" camera={{ position: [0, 0, 10] }}>
                <instancedMesh ref={particlesMeshRef} args={[undefined, undefined, settings.particleAmount]}>
                </instancedMesh>
